        ...
        <header className="mb-4 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-8">
              <a href="/" className="text-4xl font-bold tracking-tight">AIFinView</a>
            ...
            <div className="flex items-center gap-2">
              <input readOnly value="搜索股票 / 行业 / 研报 / 指标 / 宏观" className="w-[330px] rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-800" />
              <ThemeToggle />
            </div>
          </div>
        </header>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_470px]">
                <div>
                  <h1 className="text-6xl font-bold leading-tight tracking-tight">机构级中文研报，<br />证据驱动，拒绝幻觉</h1>
                  <p className="mt-3 text-lg text-slate-600 dark:text-slate-300">覆盖中 / 港 / 美主要市场，结合多源数据与严格验证体系，为机构投资者提供可验证、可追溯、可复用的研究结论。</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <input value={stockInput} onChange={(e) => setStockInput(e.target.value)} placeholder="输入股票名称 / 代码 / 简称（AAPL / 00700 / 600519）" className="min-w-[320px] flex-1 rounded-xl border border-slate-300 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800" />
                  </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                  ...
                  <div className="grid grid-cols-[150px_minmax(0,1fr)] gap-3">
                    <div className="flex h-36 w-36 items-center justify-center rounded-full border-[10px] border-emerald-500 bg-white text-center dark:bg-slate-900">
                      <div><p className="text-6xl font-bold leading-none">96</p><p className="text-xl text-slate-500">/100</p></div>
                    </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" id="agent-workflow">
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-4xl font-semibold">7-Agent 协作工作流</h2>
